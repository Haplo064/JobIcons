...
namespace JobIcons2
{
    internal class IconSet
    {
        ...
        public static void Initialize(JobIcons2Plugin plugin)
        {
            ...
            Add("Custom1", job => plugin.Configuration.CustomIconSet1[(int)job]);
            Add("Custom2", job => plugin.Configuration.CustomIconSet2[(int)job]);
        }

        private const float DefaultScaleMultiplier = 1;

        public static IconSet Get(string name) => IconSets[name];

        public static string[] Names => IconSets.Keys.ToArray();

        private static void Add(string name, int[] icons, float scaleMultiplier = DefaultScaleMultiplier)
        {
            IconSets[name] = new IconSet(name, icons, scaleMultiplier);
        }

        private static void Add(string name, Func<Job, int> iconMethod, float scaleMultiplier = DefaultScaleMultiplier)
        {
            IconSets[name] = new IconSet(name, iconMethod, scaleMultiplier);
        }

        private static readonly Dictionary<string, IconSet> IconSets = new();
        ...
        private readonly Func<Job, int> _iconMethod;
        ...
        public int GetIconId(uint jobId)
        {
            ...
            if (_iconMethod != null)
            {
                ...
                return _iconMethod(job);
            }

            if (_icons != null)
                return _icons[(int)job];
