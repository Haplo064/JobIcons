...
internal class JobIcons2Gui : IDisposable
{
    ...
    private static bool OnBuildUi_IconSetChoice(string label, string currentName, Action<string> setIconSetName)
    {
        var iconSetNames = IconSet.Names;
        var index = Array.IndexOf(iconSetNames, currentName);
        if (!ImGui.Combo(label, ref index, iconSetNames, iconSetNames.Length)) return false;
    ...
    private void UpdateNamePlates()
    {
        ...
        for (var i = 0; i < 50; i++)
        {
            ...
            var scaleMulti = iconSet.ScaleMultiplier;

            npObject.SetIconScale(Configuration.Scale * scaleMulti);
